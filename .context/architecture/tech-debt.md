...
| ID | Debt | Affected files | Epic |
|---|---|---|---|
| BUG-03 | **Broken redirect targets**: feature/plan middlewares redirect to `/upgrade` (no such page), permission middleware to `/dashboard` (home is `/`), `UpgradePrompt` navigates to `/billing?plan=…` (no such page), and Stripe checkout `cancel_url` points to `${baseUrl}/pricing` (404). | `layers/entitlements/middleware/feature.ts` (L33), `requirePlan.ts` (L32), `permission.ts` (L32), `layers/entitlements/components/UpgradePrompt.vue` (L34), `layers/billing/server/api/billing/checkout.post.ts` (L143) | E02 (fix targets); E05/E06 build the real destinations |
