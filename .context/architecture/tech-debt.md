...
| ID | Debt | Affected files | Epic |
|---|---|---|---|
| BUG-03 | **Broken redirect target**: Stripe checkout `cancel_url` points to `${baseUrl}/pricing` (404). (Entitlements middleware/UpgradePrompt targets repointed at `/settings/billing`/`/` in T3.) | `layers/billing/server/api/billing/checkout.post.ts` (L143) | E02 (fix target); E05 builds the real /pricing |
