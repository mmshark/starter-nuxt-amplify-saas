...
> **Status**: Active · **Created**: 2026-07-08 · **Source**: new (work queue for [spec.md](./spec.md) per [plan.md](./plan.md))
...
- [ ] Introduce a single canonical cookie name `current-workspace-id` as an exported constant in
...
- [ ] `layers/entitlements/server/utils/getWorkspaceContext.ts:51` — read the canonical name.
- [ ] `layers/workspaces/composables/useWorkspaces.ts` — add a `setCurrentWorkspace(id)` helper that
...
- [ ] Update the doc comments naming the old cookie: `getWorkspaceContext.ts:41,76`,
...
- [ ] Extend `layers/workspaces/composables/__tests__/useWorkspaces.test.ts` to assert the cookie is
      written with the canonical name on auto-select and switch.
- [ ] Ledger: delete row BUG-01.
...
- [ ] `layers/workspaces/server/api/workspaces/index.get.ts` — after mapping workspaces (`:47-57`),
...
- [ ] Extend `layers/workspaces/composables/__tests__/useWorkspaces.test.ts`: a workspace envelope
...
- [ ] `layers/entitlements/composables/useEntitlements.ts:22` — destructure `currentUser` from
      ...
      (BUG-15, spec item 10).
- [ ] Ledger: delete rows BUG-02 and BUG-15.
...
- [ ] `layers/entitlements/middleware/feature.ts:33` — `/upgrade` → `/settings/billing` (keep
      `feature`/`plan` query).
- [ ] `layers/entitlements/middleware/requirePlan.ts:32` — `/upgrade` → `/settings/billing` (keep
      `plan` query).
- [ ] `layers/entitlements/middleware/permission.ts:32` — `/dashboard` → `/` (keep
      `error=insufficient_permissions` query).
- [ ] `layers/entitlements/components/UpgradePrompt.vue:34` — `/billing?plan=…` →
      `/settings/billing?plan=…`.
- [ ] Ledger: narrow row BUG-03 to the checkout `cancel_url` part (deleted fully by T4; whichever of
...
- [ ] `layers/billing/server/api/billing/checkout.post.ts:143` — `${baseUrl}/pricing` →
...
- [ ] Ledger: delete row BUG-03 (if T3 already landed; otherwise narrow).
...
- [ ] `layers/auth/components/UserAccountForm.vue` — make the email input `readonly disabled`
...
- [ ] `layers/saas/pages/profile/account.vue` — correct the card description ("Manage your account
      settings, email, and password" promises what doesn't exist).
- [ ] Ledger: update row SEC-06 — stopgap applied, E07 remainder (verified change flow) stays.
...
- [ ] `layers/saas/pages/profile/security.vue` — replace the handler-less password `UForm` (`:32-57`)
      with guidance + link to the working reset flow (`/auth/forgot-password`); drop the unused
      zod schema/state.
- [ ] Remove the delete-account card (`:60-68`) — or keep it with the button disabled and an explicit
      "Not yet available" label (removal preferred, see plan D4).
- [ ] Ledger: update row BUG-04 — lying surface removed, E07 remainder (real password change +
...
- [ ] `apps/backend/amplify/seed/seeders/users.ts:341` — gate on `if (user.planId)` and pass
      `user.billingInterval ?? 'month'` to `createWorkspaceSubscription` (signature at `:65`).
- [ ] `apps/backend/amplify/seed/data/users.json` — add explicit `"billingInterval": "month"` to the
      four paid users (`pro`/`enterprise`), keeping the fixture self-describing.
- [ ] Ledger: delete row BUG-06.
...
- [ ] `apps/backend/package.json:19` — remove `sandbox:amplify:seed`.
- [ ] Root `package.json:15` — remove `backend:sandbox:amplify:seed`.
- [ ] `apps/backend/README.md:121` — remove the script's row.
- [ ] `.context/operations/environments.md:90` and `.context/operations/debugging.md:19` — remove the
      now-obsolete "broken seed script" workaround notes.
- [ ] Ledger: delete row BUG-07.
...
- [ ] `layers/debug/pages/debug/index.vue:55` — replace the `appConfig.billing?.plans` computed with
...
- [ ] Drop the now-unused `appConfig` billing references (keep other `useAppConfig` uses if any).
- [ ] Ledger: delete row BUG-08.
...
- [ ] Verify E01's fixes cleared `layers/workspaces/server/**` (grep below → 0 lines).
- [ ] `server/middleware/auth.ts:2` — only if the `aws-amplify/auth/server` TS2307 survives E01's fix
...
- [ ] Ledger: no dedicated row (BUG-09 is E01's); record the verification outcome in the T11
...
- [ ] Run the deferred **[sandbox]** verifications (T2, T4, T7, T9) against a live sandbox with
      seeded Stripe plans; record results in the commit body.
- [ ] `.context/prd/roadmap.md` — set E02 status to `done`.
- [ ] Add `.context/changelogs/<date>-fix-broken-wiring.md` summarizing what was fixed and which
      stopgaps E07/E05/E06 must supersede.
- [ ] Confirm ledger state: BUG-01/02/03/06/07/08 rows gone; SEC-06/BUG-04 narrowed to E07 remainder.

**Verify**:
```sh
grep -n "E02" .context/prd/roadmap.md                       # status done
