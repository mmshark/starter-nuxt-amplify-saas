...
- [x] **T01 — Commit stub outputs + gitignore exception + copy script**
...
- [x] **T02 — Declare missing type-level deps in the amplify layer** (deps: T01)
...
- [x] **T03 — Fix saas-layer config export resolution** (deps: T01)
...
- [x] **T04 — Fix short relative type imports in workspaces server routes** (deps: T01)
...
- [x] **T06 — Fix residual errors in `layers/workspaces/server/**`** (deps: T05)
...
- [x] **T07a — Fix residual errors in `layers/billing/server/api/billing/*.ts`** (deps: T05)
...
- [x] **T07b — Fix residual errors in `layers/entitlements/server/utils/getWorkspaceContext.ts`** (deps: T05)
...
- [x] **T07c — Fix residual errors in `layers/debug/pages/debug/*.vue`** (deps: T05)
...
- [x] **T07d — Fix residual errors in `layers/auth/**`** (deps: T05)
...
- [x] **T07e — Fix residual errors in `apps/saas/app/**`; zero-errors gate** (deps: T05, T06, T07a–T07d)
...
- [x] **T08 — Declare Node ≥22 across the repo** (deps: none, parallel-safe)
...
- [x] **T09 — Add stub step to ci.yml** (deps: T01)
...
- [x] **T10 — Clean-clone rehearsal, no AWS env** (deps: T05–T09; no commit — verification gate)
...
- [x] **T11 — Conditional: fix landing generate on clean checkout** (deps: T10, only if it failed there)
...
- [x] **T12 — PR to master; first real CI run must be green** (deps: all above)
...
- [x] **T13 — Merge; verify green on master; close out docs** (deps: T12)
  Files: [../../roadmaps/20260711-saas-boilerplate-productization.md](../../roadmaps/20260711-saas-boilerplate-productization.md) (E01 status → done),
