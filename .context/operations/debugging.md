...
| `typecheck`/`build` fails with TS2307 `Cannot find module '../amplify_outputs.json'` (~341 TS errors) | `layers/amplify/plugins/01.amplify.client.ts`, `01.amplify.server.ts` and `layers/amplify/server/utils/amplify.ts` import `amplify_outputs.json` statically; no stub exists, so a clean checkout cannot typecheck or pass CI | Same as above: generate outputs from a live sandbox first. Decoupling/stubbing is roadmap epic E01 (green-ci) |
...
| `/debug` shows "No billing plans found in app configuration"; Test Checkout disabled | `layers/debug/pages/debug/index.vue` reads `appConfig.billing?.plans`, which no layer defines since the plans-from-Stripe migration — the selector is always empty | Known broken (roadmap E02 fix-broken-wiring). Use `/debug/plans` (reads the real `SubscriptionPlan` table) instead |
| Entitlements always resolve to free plan / `user` role despite an active subscription | Cookie name mismatch: `layers/workspaces/composables/useWorkspaces.ts` writes `current-workspace-id`, but `layers/entitlements/server/utils/getWorkspaceContext.ts` reads `currentWorkspaceId` — the cookie fallback never matches (fail-closed) | Pass `workspaceId` explicitly to entitlement checks. Fix is roadmap E02 |
| `pnpm backend:sandbox:amplify:seed` fails: cannot find `scripts/amplify-seed.ts` | `apps/backend/package.json` script points at `apps/backend/scripts/`, which does not exist | Use the working seeders: `pnpm backend:sandbox:seed`, `backend:sandbox:seed:plans`, `backend:sandbox:seed:users` (roadmap E02) |
