...
1. **Set `STRIPE_WEBHOOK_SECRET`** — `task sandbox:secrets` for the sandbox; Amplify Console → App Settings → Secrets for deployed branches.
...
- **Offline outputs are solved for CI.** E01 added a committed stub and `task sandbox:outputs:stub`,
  so clean-checkout lint/typecheck/build succeeds. Real deployments must still generate outputs from
  the deployed backend before frontend builds.
...
- GitHub Actions CI runs `task ci:all` and is green on `master`. The two publish workflows are
  independent of app deployment and still require their own release-path verification.
