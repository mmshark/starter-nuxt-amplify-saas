  ...
  try {
    const authContext = await withAmplifyAuth(event, async (contextSpec) => {
      try {
        ...
        const attributes = session.tokens ? await fetchUserAttributes(contextSpec) : null
        return { user, session, attributes }
      } catch (e) {
        return { user: null, session: null, attributes: null }
      }
    })
    ...
    event.context.userAttributes = authContext.attributes
  } catch (error) {
